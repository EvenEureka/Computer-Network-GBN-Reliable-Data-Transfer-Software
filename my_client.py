'''
client端应该实现的功能:
    根据GBN协议,维护发送窗口。根据发送方的地址,封装好PDU数据帧头部和PDU数据帧。根据数据生成checksum。发送PDU给server。
    根据GBN协议,维护接收窗口。收到PDU数据帧后,提取出头部信息，检查序号是否正确，并检查数据是否出错。
    如果数据出错，则丢弃帧。如果数据没出错但是序号不对,也丢弃。如果都没错,则正常接收,并向发送方返回ack帧。
    正确识别接收方的ack帧,维护自己的窗口。
'''
import my_timer
import my_PDU
import my_crc
