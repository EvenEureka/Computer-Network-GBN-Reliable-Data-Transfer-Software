...
class MyPDUHeader:
    ...
    前三个是header_length(2B), data_length(2B), crc_num(2B),共6字节
    然后是sourceIp(4B)和sourcePort(2B),destIp(4B)和destPort(2B),共12字节。
    最后是type(指定是数据帧还是ack帧,2B),seq_num(4B)
    ...
    def to_byte(self)->bytes:
        # 将Header打包成config.HeaderLength字节的字节流
        # e.g. 下行:2 表示要转换为两个字节，config.EndianType 指定了字节序，config.isSigned 指定了是否为有符号数。
        header_length=self.header_length.to_bytes(2, config.EndianType, signed=config.isSigned)
        data_length=self.data_length.to_bytes(2, config.EndianType, signed=config.isSigned) # 虽然现在还不知道，但是先转化为字节流，然后在PDU中改
...
class MyPDU:
    def __init__(self, header:MyPDUHeader):
        ...
        self.data_length=header.data_length # 数据部分长度
